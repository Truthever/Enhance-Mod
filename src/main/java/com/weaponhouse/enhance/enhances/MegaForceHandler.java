...
public class MegaForceHandler {
    ...
    private static final float DAMAGE_BONUS_PER_LEVEL = 0.05f;
    private static final float ARROW_SPEED_BONUS_PER_LEVEL = 0.025f;
    ...
    public static void onLivingHurt(LivingHurtEvent event) {
        ...
        if (source.isExplosion()) {
            ...
            if (explosionSource != null) {
                int level = getMegaForceLevel(explosionSource);
                if (level > 0) {
        ...
        if (source.getImmediateSource() instanceof AbstractArrowEntity) {
            ...
            if (arrow.getShooter() instanceof LivingEntity) {
                ...
                int level = getMegaForceLevel(shooter);
                if (level > 0) {
                    float originalDamage = event.getAmount();
                    float newDamage = originalDamage * (1 + level * DAMAGE_BONUS_PER_LEVEL);
                    event.setAmount(newDamage);
    ...
    private static void applyMeleeBonus(LivingEntity attacker, LivingDamageEvent event) {
        int level = getMegaForceLevel(attacker);
        if (level > 0) {
    ...
    private static int getMegaForceLevel(LivingEntity entity) {
        if (entity.getPersistentData().contains(BUFF_TAG)) {
            ...
            if (buffs.contains(MEGAFORCE_TAG)) {
                return buffs.getInt(MEGAFORCE_TAG);
            }
        }
        return 0;
    }
