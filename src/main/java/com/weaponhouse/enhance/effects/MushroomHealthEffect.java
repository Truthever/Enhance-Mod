...
public class MushroomHealthEffect extends BaseEffect {
    ...
    public void applyEffect(LivingEntity entity, int amplifier) {
        ...
        applyAttributeModifier(maxHealthAttr, adjustment);
    ...
    public void removeEffect(LivingEntity entity, int amplifier) {
        ...
        if (maxHealthAttr == null) return;
        CompoundNBT nbt = entity.getPersistentData();
        removeAttributeModifier(maxHealthAttr);
        if (nbt.contains("OriginalMaxHealth")) {
    ...
    private void applyAttributeModifier(ModifiableAttributeInstance attribute, double amount) {
        AttributeModifier existingModifier = attribute.getModifier(MushroomHealthEffect.MAX_HEALTH_MODIFIER_UUID);
        if (existingModifier != null) {
            attribute.removeModifier(existingModifier);
        }
        AttributeModifier newModifier = new AttributeModifier(MushroomHealthEffect.MAX_HEALTH_MODIFIER_UUID, "mushroom_health_boost", amount, AttributeModifier.Operation.ADDITION);
