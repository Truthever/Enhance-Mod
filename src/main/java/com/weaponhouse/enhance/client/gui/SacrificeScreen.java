...
public class SacrificeScreen extends Screen {
    ...
    public static final String KEY_SCREEN_TITLE_LINE1 = "gui.enhance.sacrifice.title.line1";
    public static final String KEY_SCREEN_TITLE_LINE2 = "gui.enhance.sacrifice.title.line2";
    ...
    private void renderBuffInfo(MatrixStack matrixStack, int guiLeft, int guiTop) {
        ...
        for (int i = 0; i < BUFF_AREA_COORDINATES.length; i++) {
            ...
            int width = x2 - x1;
            int height = y2 - y1;
            renderSingleBuff(matrixStack, guiLeft, guiTop, buffInfo.name, buffInfo.level, x1, y1, width, height, buffInfo.color);
        }
    }
    ...
    private String getOriginalBuffName(String buffString) {
        if (buffString == null || buffString.isEmpty()) {
            return "";
        }
        String[] parts = buffString.split("Lv.");
    ...
    private boolean isMouseOverButton(int buttonIndex, double mouseX, double mouseY, int guiLeft, int guiTop) {
        if (buttonIndex < 0 || buttonIndex >= BUTTON_COORDINATES.length) {
            return false;
        }
        if (!hasBuffAtPosition(buttonIndex)) {
            return false;
        }
        ...
        return mouseX >= btnX1 && mouseX <= btnX2 &&
                mouseY >= btnY1 && mouseY <= btnY2;
    }
    @Override
    ...
    private void sacrificeBuffAtPosition(int buttonIndex) {
        ...
        try {
            ...
            boolean isSacrificeBuff = EnhanceSacrificeRules.isSacrificeableBuff(buffId);
            if (isSacrificeBuff) {
            ...
            Minecraft.getInstance().player.sendMessage(
                    new StringTextComponent(TextFormatting.RED + I18n.format(KEY_EXCEPTION_REMOVE)),
