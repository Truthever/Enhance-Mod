...
public class EnhanceDisplayScreen extends Screen {
    ...
    public void updateBuffs(List<String> buffs) {
        ...
        if (totalBuffs == 0) {
            buffInfos.add(new BuffInfo(I18n.format("gui.enhance.display.no_buff"), "", "", 0xFFFFFF));
        }
    ...
    private void renderText(MatrixStack matrixStack, int guiLeft, int guiTop) {
        ...
        String playerName = player != null ? player.getName().getString() : I18n.format("gui.enhance.display.unknown_player");
    ...
    private void removeBuffAtPosition(int index) {
        ...
        if (info.originalName.isEmpty() || info.name.equals(I18n.format("gui.enhance.display.no_buff"))) return;
        PlayerEntity player = Minecraft.getInstance().player;
        if (player == null) return;
        String tip = I18n.format(KEY_SACRIFICE_TIP);
        player.sendMessage(
                new StringTextComponent(TextFormatting.YELLOW + tip),
                player.getUniqueID()
        );
    }
    private boolean hasBuffAtPosition(int index) {
        ...
        BuffInfo info = buffInfos.get(index);
        return !info.originalName.isEmpty() && !info.name.equals(I18n.format("gui.enhance.display.no_buff"));
    }
    ...
    private void renderBuffInfo(MatrixStack matrixStack, int guiLeft, int guiTop) {
        ...
        for (int i = 0; i < BUFF_AREA_COORDINATES.length; i++) {
            ...
            if (totalBuffs == 0 && i == 0) {
                buffInfo = new BuffInfo(I18n.format("gui.enhance.display.no_buff"), "", "", 0xFFFFFF);
            }
